...
def Test(test_file, lang=None):
    ...
    return good/(good+bad)*100.00, UAS, LS, LAS
...
def set_class_map(data):
    meta.bhc2i, meta.ec2i= [{'bos':0, 'eos':1, 'unk':2}]*2
    bhcid, ecid = len(meta.bhc2i), len(meta.ec2i)
    for graph in data:
        for pnode in graph[1:-1]:
            for c in pnode.form:
                if pnode.lang in ['hi', 'bn']:
                    if c not in meta.bhc2i:
                        meta.bhc2i[c] = bhcid
                        bhcid += 1
                else:
                    if c not in meta.ec2i:
                          meta.ec2i[c] = ecid
                          ecid += 1
...
if __name__ == "__main__":
    ...
    parser.add_argument('--btrain', help='Bengali CONLL/TNT Train file')
    parser.add_argument('--htrain', help='Hindi CONLL/TNT Train file')
    parser.add_argument('--hdev', help='Hindi CONLL/TNT Dev/Test file')
    parser.add_argument('--bdev', help='Bengali CONLL/TNT Dev/Test file')
    parser.add_argument('--hcdev', help='Hindi-English CS CONLL/TNT Dev/Test file')
    parser.add_argument('--bcdev', help='Bengali-English CS CONLL/TNT Dev/Test file')
    ...
    parser.add_argument('--clusters', dest='clusters', help='Cluster Projections')
	...
	#train_sents_hin = read(args.htrain, 'hi')
        train_sents_ben = read(args.btrain, 'bn')
        train_sents_eng = read(args.etrain, 'en')
        train_sents_hin = read(args.htrain, 'hi')

        set_class_map(train_sents_ben+train_sents_eng +train_sents_hin)
        ...
        bwvm = KeyedVectors.load_word2vec_format(args.bembd, binary=args.bvec, limit=args.hlimit)
        ...
        meta.n_words_ben = bwvm.syn0.shape[0]+meta.add_words
    ...
    if args.load_model:
        ...
        train_parser(train_sents_ben+train_sents_eng +train_sents_hin)
