...
def test_raw_sents(test_file, ofp):
...
def Test(test_file, ofp=None, lang=None):
	...
	pred_pos = []
        ...
        print '\n'.join(['\t'.join([unicode(node.id), node.form, u'_', pos, u'_', u'_', unicode(node.pparent),
                      node.pdrel.strip('%'), u'_', u'_']) for node,pos in zip(dgraph, pred_pos)])
...
def test_raw_sents(parser, test_file, ofp):
    ...
    for i,sentence in enumerate(inputGenTest):
        print i
        parsed_sent = parse_sent(parser, sentence.group(1))
        ofp.write(parsed_sent+'\n\n')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--bctrain', help='Bengali-English CS CONLL Train file')
	...
	with io.open(args.outfile, 'w', encoding='utf-8') as ofp:
                if args.test:
                    ...
                    for dfile in args.cdev:
                        POS, UAS, LS, LAS = test_conll(parser, dfile, ofp)
                        sys.stderr.write("TEST-SET POS: {}%, UAS: {}%, LS: {}% and 
